...
`sails.pay.connect` is for platforms that hold money for other people: creators, maintainers, contractors, sellers. Each person gets a **connected account** with a balance of its own. You collect payments centrally, transfer each person's share into their balance, and they withdraw to their bank.
...
`sails.pay.connect` requires `sails-pay` 0.2.3 and `@sails-pay/bachs` 0.0.4 or later.
:::
...
Your platform fee is whatever stays in your platform balance after the transfer. If a customer pays ₦10,500 and you transfer ₦10,000, the ₦500 left over is yours. You don't need a separate account for it.
...
A sponsorship platform where patrons fund maintainers and the platform keeps 5%:

```js
// 1. Checkout: the patron pays the sponsorship plus the platform fee
const checkoutUrl = await sails.pay.checkout({
  items: [{ product: productId, amount: '10500.00' }],
  reference: sponsorship.reference,
  metadata: { sponsorship: sponsorship.reference }
})

// 2. Webhook: once the collection succeeds, move the maintainer's share
...
// 3. Later, the maintainer withdraws
